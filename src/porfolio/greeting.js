import React from "react";

//Home Page
const greeting = {
  title: <p className="fancy-link"> E-Ro Nguyen </p>,
  logo_name: "_n_e_R_O",
  nickname: "Ro",
  subTitle: (
    <>
      <p>
        I am currently a second-year CS Ph.D. student at{" "}
        <a
          href="https://www.cs.stonybrook.edu/"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link2"
        >
          SUNY Stony Brook
        </a>{" "}
        working with{" "}
        <a
          href="http://michaelryoo.com"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link"
        >
          Prof. Michael Ryoo
        </a>{" "}
        and{" "}
        <a
          href="https://scholar.google.com/citations?user=BxbKTYkAAAAJ&hl=en"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link"
        >
          Prof. Dimitris Samaras
        </a>
        .
      </p>
      <p>
        Prior that, I am obtained my B.Sc at{" "}
        <a
          href="https://hcmus.edu.vn"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link2"
        >
          University of Science - VNU-HCM, Vietnam
        </a>{" "}
        advised by{" "}
        <a
          href="https://scholar.google.com/citations?user=lt2ATkkAAAAJ&hl=en"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link"
        >
          {" "}
          Prof. Minh-Triet Tran
        </a>{" "}
        and was an Research Intern at{" "}
        <a
          href="https://www.qualcomm.com/research/artificial-intelligence/ai-residency-program"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link2"
        >
          Qualcomm AI Research (former VinAI Research)
        </a>
        , advised by{" "}
        <a
          href="https://researchers.adelaide.edu.au/profile/minhhoai.nguyen"
          target="_blank"
          rel="noopener noreferrer"
          className="fancy-link"
        >
          {" "}
          Prof. Minh Hoai
        </a>
        .
      </p>
      <p>
      My primary research focus lies in vision-language understanding and robotics learning. 
      </p>
    </>
  ),
  resumeLink:
    "https://drive.google.com/file/d/1I2KV3iM4CkpsEbaeMkkDvOc1s1T78Msp/view?usp=drive_link",
  portfolio_repository: "https://github.com/nero1342",
  githubProfile: "https://github.com/nero1342",
};

export default greeting;
